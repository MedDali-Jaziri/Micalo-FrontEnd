...
export class AddWorkerIntoOffreComponent implements OnInit {
  ...
  onEnter(valueModi: string) {
    ...
    this.http.post('http://20.124.200.128/company/getOffreById', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
  ...
  addToOffre() {
    ...
    if (!this.form.valid) {
      ...
      this.http.post('http://20.124.200.128/company/Step1Offre', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              if (val == 1) {
                ...
                Swal.fire({
                  icon: 'error',
                  title: 'Oops...',
                  text: "CYou can't add the same worker!",
                })
              }
            }
          }
  ...
  editAnWorkerFromOffre(workerName: any, idWorkerNameLocal: any, FGLocal: any, nbWorkOnCompanylocal: any, nbWorkOnSiteLocal: any) {
  ...
  updateInformationClientTODB() {
    ...
    this.http.post('http://20.124.200.128/company/updateInformationOfStep1', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          this.visible3 = !this.visible3
  ...
  deleteAnWorkerFromOffre(workerProfile: any, imageProfile: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://20.124.200.128/company/deleteAnWorkerFromOffre', queryObj)
          .subscribe(res => {
            for ([key, val] of Object.entries(res)) {
              if (key == "resutFunction") {
                if (val == 1) {
