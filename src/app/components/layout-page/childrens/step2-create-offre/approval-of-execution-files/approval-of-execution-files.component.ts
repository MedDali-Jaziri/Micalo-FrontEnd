...
export class ApprovalOfExecutionFilesComponent implements OnInit {
  ...
  onEnter(valueModi: string) {
    ...
    this.http.post('http://20.124.200.128/company/getOffreStep2ById', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
  ...
  addToOffre() {
    ...
    if (!this.form.valid) {
      ...
      this.http.post('http://20.124.200.128/company/Step2Offre', queryObj)
        .subscribe(res => {
          for ([key, val] of Object.entries(res)) {
            if (key == "resutFunction") {
              if (val == 1) {
  ...
  deleteAnWorkerFromOffre(workerProfile: any, imageProfile: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.http.post('http://20.124.200.128/company/deleteAnWorkerFromOffreStep2', queryObj)
          .subscribe(res => {
            for ([key, val] of Object.entries(res)) {
              if (key == "resutFunction") {
                if (val == 1) {
  ...
  editAnWorkerFromOffreStep2(nbWorkOnCompanyLocal: any, nbWorkOnSiteLocal: any, FGLocal: any, workerNameGlobaleLocal: any, idWorker: any) {
  ...
  updateInformationClientTODB() {
    ...
    this.http.post('http://20.124.200.128/company/updateInformationOfStep2', queryObj)
      .subscribe(res => {
        for ([key, val] of Object.entries(res)) {
          this.visible3 = !this.visible3
          this.onEnter(this.idOffreSendGlobale)
